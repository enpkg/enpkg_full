...
parser.add_argument(
    "-o", "--pathout", required=True, help="The path to export the resulting .ttl file"
)
...
for _, row in tqdm(df_bio_metadata.iterrows(), total=len(df_bio_metadata)):
    ...
    document_id_uri = rdflib.term.URIRef(
        document_chembl_url + row["document_chembl_id"]
    )
    ...
    g.add(
        (
            rdflib.term.URIRef(kg_uri + row["short_inchikey"]),
            ns_kg.is_InChIkey2D_of,
            uri_ik,
        )
    )
    g.add(
        (rdflib.term.URIRef(kg_uri + row["short_inchikey"]), RDF.type, ns_kg.InChIkey2D)
    )
    ...
    g.add(
        (
            target_id_uri,
            ns_demo.target_name,
            rdflib.term.Literal(row["target_pref_name"]),
        )
    )
    ...
    g.add(
        (
            compound_activity_uri,
            ns_demo.activity_type,
            rdflib.term.Literal(row["standard_type"]),
        )
    )
    g.add(
        (
            compound_activity_uri,
            ns_demo.activity_relation,
            rdflib.term.Literal(row["standard_relation"]),
        )
    )
    g.add(
        (
            compound_activity_uri,
            ns_demo.activity_value,
            rdflib.term.Literal(row["standard_value"], datatype=XSD.float),
        )
    )
    g.add(
        (
            compound_activity_uri,
            ns_demo.activity_unit,
            rdflib.term.Literal(row["standard_units"]),
        )
    )
    ...
    g.add(
        (
            document_id_uri,
            ns_demo.journal_name,
            rdflib.term.Literal(row["document_journal"]),
        )
    )
    g.add(
        (
            ...
            RDFS.label,
            rdflib.term.Literal(
                f"{row['standard_type']} of {row['molecule_chembl_id']} in assay {row['assay_chembl_id']} against {row['target_chembl_id']} ({row['target_pref_name']})"
            ),
        )
    )
