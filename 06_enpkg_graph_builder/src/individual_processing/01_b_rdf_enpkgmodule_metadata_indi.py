...
for directory in tqdm(samples_dir):
    ...
    try:
        ...
        if metadata.sample_type[0] == "sample":
            ...
            plant_parts = metadata[
                [
                    "organism_organe",
                    "organism_broad_organe",
                    "organism_tissue",
                    "organism_subsystem",
                ]
            ].copy()
            ...
            g.add(
                (
                    material_id,
                    ns_module.has_organe,
                    rdflib.term.URIRef(module_uri + plant_parts["organism_organe"][0]),
                )
            )
            g.add(
                (
                    material_id,
                    ns_module.has_broad_organe,
                    rdflib.term.URIRef(
                        module_uri + plant_parts["organism_broad_organe"][0]
                    ),
                )
            )
            g.add(
                (
                    material_id,
                    ns_module.has_tissue,
                    rdflib.term.URIRef(module_uri + plant_parts["organism_tissue"][0]),
                )
            )
            g.add(
                (
                    material_id,
                    ns_module.has_subsystem,
                    rdflib.term.URIRef(
                        module_uri + plant_parts["organism_subsystem"][0]
                    ),
                )
            )
            ...
            for assay_id, target, chembl_id, rdfclass in zip(
                ...
                [
                    ns_module.Ldono10ugml,
                    ns_module.Ldono2ugml,
                    ns_module.Tbrucei10ugml,
                    ns_module.Tbrucei2ugml,
                    ns_module.Tcruzi10ugml,
                    ns_module.L610ugml,
                ],
            ):

                assay = rdflib.term.URIRef(
                    module_uri + metadata.sample_id[0] + "_" + target
                )
                ...
                g.add(
                    (
                        ...
                        RDFS.label,
                        rdflib.term.Literal(
                            f"{target} assay of {metadata.sample_id[0]}"
                        ),
                    )
                )
                g.add(
                    (
                        assay,
                        ns_module.inhibition_percentage,
                        rdflib.term.Literal(metadata[assay_id][0], datatype=XSD.float),
                    )
                )
        ...
        pathout = os.path.normpath(
            os.path.join(pathout, f"metadata_module_enpkg.{output_format}")
        )
