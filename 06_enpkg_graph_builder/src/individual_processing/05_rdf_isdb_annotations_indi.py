...
for directory in tqdm(samples_dir):
    ...
    for _, row in isdb_annotations.iterrows():
        ...
        g.add(
            (has_isdb_annotation_hash, ns_kg.has_content, rdflib.term.Literal(data_1))
        )
        del (hash_1, data_1)
        ...
        g.add(
            (
                ...
                rdflib.term.Literal(row["score_max_consistency"], datatype=XSD.float),
            )
        )
        g.add(
            (
        ...
        g.add(
            (isdb_annotation_id, ns_kg.has_adduct, rdflib.term.Literal(row["adduct"]))
        )
