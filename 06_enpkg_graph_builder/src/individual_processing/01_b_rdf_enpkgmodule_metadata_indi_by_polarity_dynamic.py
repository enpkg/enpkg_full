...
source_id_header = params_list_full['graph-builder'].get('source_id_header', 'organism_pf_code')
...
def process_directory(directory):
    ...
    try:
        ...
        if metadata.sample_type[0] == 'sample':
            ...
            # Process all columns dynamically, except polarity-specific ones
            for column in metadata.columns:
                if column not in [f'sample_filename_pos', f'sample_filename_neg', f'sample_injection_date_pos', f'sample_injection_date_neg', 'sample_id', 'sample_type']:
                    ...
                    if pd.notna(value):
                        ...
                        print(f"Added generic triple for {column_name} with value {value_clean}")

            # Add polarity-specific triples
            polarity_columns = {
                f'sample_filename_{ionization_mode}': f'has_sample_filename_{ionization_mode}',
                f'sample_injection_date_{ionization_mode}': f'has_sample_injection_date_{ionization_mode}',
            }

            for column, predicate in polarity_columns.items():
                if column in metadata.columns:
                    value = metadata[column][0]
                    if pd.notna(value):
                        value_clean = str(value).replace(' ', '_')
                        g.add((material_id, getattr(ns_module, predicate), Namespace(module_uri)[value_clean]))
                        print(f"Added polarity-specific triple for {predicate} with value {value_clean}")

        pathout = os.path.join(sample_dir_path, directory, "rdf/")
        os.makedirs(pathout, exist_ok=True)
        pathout = os.path.join(pathout, f"metadata_enpkg_module_{ionization_mode}.{output_format}")
