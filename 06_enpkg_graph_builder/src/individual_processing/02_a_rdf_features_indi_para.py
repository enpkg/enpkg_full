...
spectrum_dashboard_prefix = params_list_full["graph-builder"][
    "spectrum_dashboard_prefix"
]
...
def process_directory(directory):
    ...
    try:
        ...
        quant_path = os.path.join(
            ...
            directory,
            ionization_mode,
            directory + "_features_quant_" + ionization_mode + ".csv",
        )
        ...
        try:
            if not os.path.isfile(quant_path) or not os.path.isfile(metadata_path):
                print(
                    f"The file : {quant_path} or {metadata_path} does not exist, skipping {directory}"
                )
            ...
            if metadata.sample_type[0] == "sample":
                ...
                area_col = [
                    col for col in quant_table.columns if col.endswith(" Peak area")
                ][0]
                max_area = quant_table[area_col].max()

                # Add feature list object to samples
                feature_list = rdflib.term.URIRef(
                    kg_uri
                ...
                if ionization_mode == "pos":
                    lc_ms = rdflib.term.URIRef(
                        kg_uri + metadata["sample_filename_pos"][0]
                    )
                    for file in [
                        directory
                        for directory in os.listdir(
                            os.path.join(path, directory, "pos")
                        )
                    ]:
                        if file.startswith(f"{directory}_lcms_processing_params_pos"):
                            lcms_processing_params_path = os.path.join(
                                path, directory, "pos", file
                            )
                elif ionization_mode == "neg":
                    lc_ms = rdflib.term.URIRef(
                        kg_uri + metadata["sample_filename_neg"][0]
                    )
                    for file in [
                        directory
                        for directory in os.listdir(
                            os.path.join(path, directory, "neg")
                        )
                    ]:
                        if file.startswith(f"{directory}_lcms_processing_params_neg"):
                            lcms_processing_params_path = os.path.join(
                                path, directory, "neg", file
                            )

                hash_1 = get_hash(lcms_processing_params_path)
                data_1 = get_data(lcms_processing_params_path)
                has_lcms_feature_list_hash = rdflib.term.URIRef(
                    kg_uri + "has_lcms_feature_list_" + hash_1
                )
                g.add((lc_ms, has_lcms_feature_list_hash, feature_list))
                g.add(
                    (
                        has_lcms_feature_list_hash,
                        RDFS.subPropertyOf,
                        rdflib.term.URIRef(kg_uri + "has_lcms_feature_list"),
                    )
                )
                g.add(
                    (
                ...
                g.add(
                    (
                        feature_list,
                        ns_kg.has_ionization,
                        rdflib.term.Literal(ionization_mode),
                    )
                )
                g.add(
                    (
                        ...
                        RDFS.comment,
                        rdflib.term.Literal(
                            f"LCMS feature list in {ionization_mode} ionization mode of {metadata.sample_id[0]}"
                        ),
                    )
                )
                ...
                for _, row in quant_table.iterrows():
                    ...
                    g.add(
                        (
                            ...
                            ns_kg.has_row_id,
                            rdflib.term.Literal(
                                int(row["row ID"]), datatype=XSD.integer
                            ),
                        )
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.has_parent_mass,
                            rdflib.term.Literal(row["row m/z"], datatype=XSD.float),
                        )
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.has_retention_time,
                            rdflib.term.Literal(
                                row["row retention time"], datatype=XSD.float
                            ),
                        )
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.has_feature_area,
                            rdflib.term.Literal(row[area_col], datatype=XSD.float),
                        )
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.has_relative_feature_area,
                            rdflib.term.Literal(
                                row[area_col] / max_area, datatype=XSD.float
                            ),
                        )
                    )
                    ...
                    g.add(
                        (
                            feature_id,
                            ns_kg.gnps_dashboard_view,
                            rdflib.URIRef(link_spectrum),
                        )
                    )

                    # add a fast Search link to the spectrum https://fasst.gnps2.org/fastsearch/ using the index and no analog search
                    link_fast_search = (
                        gnps_fast_search_prefix
                        + usi
                        + "&library_select=gnpsdata_index&analog_select=No"
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.fast_search_gnpsdata_index_no_analog,
                            rdflib.URIRef(link_fast_search),
                        )
                    )

                    # add a fast Search link to the spectrum https://fasst.gnps2.org/fastsearch/ using the index and analog search
                    link_fast_search_analog = (
                        gnps_fast_search_prefix
                        + usi
                        + "&library_select=gnpsdata_index&analog_select=Yes"
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.fast_search_gnpsdata_index_analog,
                            rdflib.URIRef(link_fast_search_analog),
                        )
                    )

                    # add a fast Search link to the spectrum https://fasst.gnps2.org/fastsearch/ using the GNPS library and no analog search
                    link_fast_search_gnps_no_analog = (
                        gnps_fast_search_prefix
                        + usi
                        + "&library_select=gnpslibrary&analog_select=No"
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.fast_search_gnpslibrary_no_analog,
                            rdflib.URIRef(link_fast_search_gnps_no_analog),
                        )
                    )

                    # add a fast Search link to the spectrum https://fasst.gnps2.org/fastsearch/ using the GNPS library and analog search
                    link_fast_search_gnps_analog = (
                        gnps_fast_search_prefix
                        + usi
                        + "&library_select=gnpslibrary&analog_select=Yes"
                    )
                    g.add(
                        (
                            feature_id,
                            ns_kg.fast_search_gnpslibrary_analog,
                            rdflib.URIRef(link_fast_search_gnps_analog),
                        )
                    )
                ...
                pathout = os.path.normpath(
                    os.path.join(pathout, f"features_{ionization_mode}.{output_format}")
                )
