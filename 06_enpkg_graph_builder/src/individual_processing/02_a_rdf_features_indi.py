    ...
    try:
        ...
        if metadata.sample_type[0] == "sample":
            ...
            feature_list = rdflib.term.URIRef(
                kg_uri + metadata.sample_id[0] + "_lcms_feature_list_" + ionization_mode
            )
            ...
            if ionization_mode == "pos":
                ...
                for file in [
                    directory
                    for directory in os.listdir(os.path.join(path, directory, "pos"))
                ]:
                ...
                for file in [
                    directory
                    for directory in os.listdir(os.path.join(path, directory, "neg"))
                ]:
            ...
            for _, row in quant_table.iterrows():
                ...
                g.add(
                    (
                        feature_id,
                        ns_kg.has_row_id,
                        rdflib.term.Literal(row["row ID"], datatype=XSD.integer),
                    )
                )
                g.add(
                    (
