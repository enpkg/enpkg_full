    ...
    try:
        ...
        if metadata.sample_type[0] == "sample":
            ...
            g.add(
                (
                    material_id,
                    ns_kg.submitted_taxon,
                    rdflib.term.Literal(metadata[source_taxon_header][0]),
                )
            )
            ...
            g.add(
                (
                    sample,
                    RDFS.label,
                    rdflib.term.Literal(f"Sample {metadata.sample_id[0]}"),
                )
            )

            # Add GNPS Dashboard link for pos & neg: only if sample_filename_pos column exists and is not NaN and MassIVE id is present
            if (
                set(["sample_filename_pos", "massive_id"]).issubset(metadata.columns)
                and ionization_mode == "pos"
            ):
                if not pd.isna(metadata["sample_filename_pos"][0]):
                    ...
                    gnps_dashboard_link = (
                        f"{gnps_dashboard_prefix}{massive_id}:{sample_filename_pos}"
                    )
                    gnps_tic_pic = (
                        f"{gnps_tic_pic_prefix}{massive_id}:{sample_filename_pos}"
                    )
                    ...
                    for file in [
                        ...
                        if file.startswith(f"{directory}_lcms_method_params_pos"):
                            lcms_analysis_params_path = os.path.join(
                    ...
                    g.add(
                        (
                            has_lcms_hash,
                            RDFS.subPropertyOf,
                            rdflib.term.URIRef(kg_uri + "has_LCMS"),
                        )
                    )
                    g.add(
                        (has_lcms_hash, ns_kg.has_content, rdflib.term.Literal(data_1))
                    )
                    del (hash_1, data_1)
                    ...
                    g.add(
                        (
                            ...
                            ns_kg.has_gnpslcms_link,
                            rdflib.URIRef(gnps_dashboard_link),
                        )
                    )
                    g.add(
                        (
                            ...
                            ns_kg.has_massive_doi,
                            rdflib.URIRef(link_to_massive),
                        )
                    )
                    g.add(
                        (
                    ...
                    g.add(
                        (
                            ...
                            FOAF.depiction,
                            rdflib.URIRef(gnps_tic_pic),
                        )
                    )

            if (
                set(["sample_filename_neg", "massive_id"]).issubset(metadata.columns)
                and ionization_mode == "neg"
            ):
                if not pd.isna(metadata["sample_filename_neg"][0]):
                    sample_filename_neg = metadata["sample_filename_neg"][0]
                    massive_id = metadata["massive_id"][0]
                    gnps_dashboard_link = (
                        f"{gnps_dashboard_prefix}{massive_id}:{sample_filename_neg}"
                    )
                    gnps_tic_pic = (
                        f"{gnps_tic_pic_prefix}{massive_id}:{sample_filename_neg}"
                    )
                    link_to_massive = f"{massive_prefix}{massive_id}"
            ...
            metadata_taxo_path = os.path.join(
                path, directory, "taxo_output", directory + "_taxo_metadata.tsv"
            )
            taxo_params_path = os.path.join(
                path, directory, "taxo_output", "params.yaml"
            )
            try:
                ...
                if not pd.isna(metadata_taxo["wd.value"][0]):
                    ...
                    g.add(
                        (
                            has_wd_id_hash,
                            RDFS.subPropertyOf,
                            rdflib.term.URIRef(kg_uri + "has_wd_id"),
                        )
                    )
                    g.add(
                        (has_wd_id_hash, ns_kg.has_content, rdflib.term.Literal(data_2))
                    )
                    ...
                    g.add(
                        (
                            material_id,
                            ns_kg.has_unresolved_taxon,
                            rdflib.term.URIRef(kg_uri + "unresolved_taxon"),
                        )
                    )
            ...
            g.add(
                (
                    sample,
                    RDFS.label,
                    rdflib.term.Literal(f"Blank {metadata.sample_id[0]}"),
                )
            )
            ...
            g.add(
                (sample, RDFS.label, rdflib.term.Literal(f"QC {metadata.sample_id[0]}"))
            )
            if (
                set(["sample_filename_pos", "massive_id"]).issubset(metadata.columns)
                and ionization_mode == "pos"
        ...
        pathout = os.path.normpath(
            os.path.join(pathout, f"metadata_enpkg.{output_format}")
        )
