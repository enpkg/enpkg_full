...
for directory in tqdm(samples_dir):
    ...
    for _, row in csi_annotations.iterrows():
        ...
        has_sirius_annotation_hash = rdflib.term.URIRef(
            kg_uri + "has_sirius_annotation_" + hash_1
        )
        g.add((feature_id, has_sirius_annotation_hash, sirius_annotation_id))
        g.add(
            (
                has_sirius_annotation_hash,
                RDFS.subPropertyOf,
                rdflib.term.URIRef(kg_uri + "has_sirius_annotation"),
            )
        )
        g.add(
            (has_sirius_annotation_hash, ns_kg.has_content, rdflib.term.Literal(data_1))
        )
        del (hash_1, data_1)
        ...
        g.add(
            (
                sirius_annotation_id,
                RDFS.label,
                rdflib.term.Literal(f"sirius annotation of {usi}"),
            )
        )
        ...
        g.add(
            (
                sirius_annotation_id,
                ns_kg.has_sirius_score,
                rdflib.term.Literal(row["SiriusScore"], datatype=XSD.float),
            )
        )
        g.add(
            (
                sirius_annotation_id,
                ns_kg.has_zodiac_score,
                rdflib.term.Literal(row["ZodiacScore"], datatype=XSD.float),
            )
        )
        g.add(
            (
                sirius_annotation_id,
                ns_kg.has_cosmic_score,
                rdflib.term.Literal(row["ConfidenceScore"], datatype=XSD.float),
            )
        )
    ...
    pathout = os.path.normpath(
        os.path.join(pathout, f"sirius_{ionization_mode}.{output_format}")
    )
