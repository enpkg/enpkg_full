...
for directory in tqdm(samples_dir):
    isdb_path_pos = os.path.join(
        path, directory, "pos", "isdb", directory + "_isdb_reweighted_flat_pos.tsv"
    )
    isdb_path_neg = os.path.join(
        path, directory, "neg", "isdb", directory + "_isdb_reweighted_flat_neg.tsv"
    )
    ...
    for sik in short_ik:
        if (sik not in metadata_short_ik) & (sik not in short_ik_in_db):
            ...
            metadata_short_ik[sik]["npc_pathway"] = row[
                "structure_taxonomy_npclassifier_01pathway"
            ].values[0]
            metadata_short_ik[sik]["npc_superclass"] = row[
                "structure_taxonomy_npclassifier_02superclass"
            ].values[0]
            metadata_short_ik[sik]["npc_class"] = row[
                "structure_taxonomy_npclassifier_03class"
            ].values[0]
...
if gnps_id is not None:
    ...
    gnps_file = os.listdir(
        os.path.join(path, "002_gnps", gnps_id, "result_specnets_DB")
    )[0]
    gnps_annotations_path = os.path.join(
        path, "002_gnps", gnps_id, "result_specnets_DB", gnps_file
    )
    short_ik_smiles_query = {}
    try:
        gnps_annotations = pd.read_csv(
            gnps_annotations_path, sep="\t", usecols=["Smiles", "INCHI"]
        )
        ...
        metadata_short_ik = get_NPC(
            short_ik_smiles_query=short_ik_smiles_query,
            db_ik=short_ik_in_db,
            processed_ik=metadata_short_ik,
        )
    except FileNotFoundError:
        pass
else:
...
for directory in tqdm(samples_dir):
    ...
    short_ik_smiles_query = pd.Series(
        sirius_annotations.smiles.values, index=sirius_annotations.InChIkey2D
    ).to_dict()
    metadata_short_ik = get_NPC(
        short_ik_smiles_query=short_ik_smiles_query,
        db_ik=short_ik_in_db,
...
df_ik_meta = (
    pd.DataFrame.from_dict(metadata_short_ik, orient="index")
    .reset_index()
    .rename(columns={"index": "short_inchikey"})
    .fillna("unknown")
)
