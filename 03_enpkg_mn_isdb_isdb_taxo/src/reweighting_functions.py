...
def taxonomical_reponderator(dt_isdb_results, min_score_taxo_ms1):
    ...
    df["score_input"] = pd.to_numeric(df["score_input"], downcast="float")
    # and we add it to the max txo score :
    df["score_input_taxo"] = df["score_taxo"] + df["score_input"]

    df["rank_spec_taxo"] = df.groupby("feature_id")["score_input_taxo"].rank(
...
def chemical_reponderator(
    clusterinfo_summary_file, dt_isdb_results, top_N_chemical_consistency
):
    ...
    dt_isdb_results["final_score"] = (
        msms_weight * dt_isdb_results["score_input"]
    ...
    print(
        "Number of annotations reweighted at the NPClassifier pathway level: "
        + str(
            len(
                dt_isdb_results[
                    (
    ...
    print(
        "Number of annotations reweighted at the NPClassifier superclass level: "
        + str(
            len(
                dt_isdb_results[
                    (
    ...
    print(
        "Number of annotations reweighted at the NPClassifier class level: "
        + str(
            len(
                dt_isdb_results[
                    (
