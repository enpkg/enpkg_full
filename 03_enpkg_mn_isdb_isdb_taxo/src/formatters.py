...
def feature_intensity_table_formatter(feature_intensity_table):
    ...
    feature_intensity_table.columns = feature_intensity_table.columns.str.replace(
        " Peak height", ""
    )
    feature_intensity_table.rename_axis("MS_filename", axis="columns", inplace=True)
    feature_intensity_table = feature_intensity_table.rename(
        columns={feature_intensity_table.columns[0]: "intensity"}
    )
