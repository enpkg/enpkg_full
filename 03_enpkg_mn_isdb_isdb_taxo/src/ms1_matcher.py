...
def ms1_matcher(
    input_df: pd.DataFrame, adducts_df: pd.DataFrame, df_metadata: pd.DataFrame
) -> pd.DataFrame:
    """Perform MS
    ...
    for feature_id, _ in tqdm(input_df.iterrows(), total=input_df.shape[0]):
        parent_mass = input_df.loc[feature_id, "mz"]
        df_0 = input_df.loc[[feature_id], ["feature_id", "mz", "component_id"]]
        df_1 = adducts_df[
            (adducts_df["min"] <= parent_mass) & (adducts_df["max"] >= parent_mass)
        ]
        df_1["key"] = feature_id
