...
def plotter_count(df_input, sample_dir, organism, treemap_chemo_counted_results_path):
    ...
    fig.update_layout(
        ...
        title_text=sample_dir
        + " ("
        + organism
        + ") "
        + "- metabolite annotation overview (size proportional to number of annotations)",
    )
    fig.update_annotations(font_size=12)
    fig.write_html(treemap_chemo_counted_results_path)


def plotter_intensity(
    df_input, feature_table, sample_dir, organism, treemap_chemo_intensity_results_path
):
    ...
    df_input = df_input.merge(
        feature_table, left_on="feature_id", right_index=True, how="left"
    )
    fig = px.treemap(
        df_input,
        path=[
    ...
    fig.update_layout(
        ...
        + organism
        + ") "
        + "- metabolite annotation overview (size proportional to the average features intensities)",
    )
    fig.update_annotations(font_size=12)
