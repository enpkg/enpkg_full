...
from helpers import (
    top_N_slicer,
    annotation_table_formatter_taxo,
    annotation_table_formatter_no_taxo,
)
...
spectral_db_pos_path = os.path.normpath(
    params_list_full["isdb"]["paths"]["spectral_db_pos_path"]
)
spectral_db_neg_path = os.path.normpath(
    params_list_full["isdb"]["paths"]["spectral_db_neg_path"]
)
adducts_pos_path = os.path.normpath(
    params_list_full["isdb"]["paths"]["adducts_pos_path"]
)
adducts_neg_path = os.path.normpath(
    params_list_full["isdb"]["paths"]["adducts_neg_path"]
)
...
top_N_chemical_consistency = params_list_full["isdb"]["reweighting_params"][
    "top_N_chemical_consistency"
]
min_score_taxo_ms1 = params_list_full["isdb"]["reweighting_params"][
    "min_score_taxo_ms1"
]
min_score_chemo_ms1 = params_list_full["isdb"]["reweighting_params"][
    "min_score_chemo_ms1"
]
...
print(
    f"{len(samples_dir)} directories were detected in the input directory. They will be checked for minimal requirements."
)
...
for sample_dir in samples_dir[:]:
    ...
    try:
        metadata_file_path = os.path.join(
            repository_path, sample_dir, sample_dir + "_metadata.tsv"
        )
    ...
    if metadata["sample_type"][0] == "sample":
        ...
        print(
            sample_dir
            + " is a QC or a blank sample, it is removed from the processing list."
        )
        samples_dir.remove(sample_dir)
        continue
    ...
    if os.path.isfile(
        os.path.join(repository_path, sample_dir, sample_dir + "_features_ms2" + ".mgf")
    ):
        pass
    else:
    ...
    if os.path.isfile(
        os.path.join(
            repository_path, sample_dir, sample_dir + "_features_quant" + ".csv"
        )
    ):
        pass
    else:
        print(
            sample_dir
            + " has no feature intensity table, it is removed from the processing list."
        )
        samples_dir.remove(sample_dir)
        continue
    if recompute == False:
        if os.path.isfile(
            os.path.join(repository_path, sample_dir, "isdb/config.yaml")
        ):
            print(
                sample_dir
                + " has already been annotated through the ISDB, since the recompute option (user.yaml) is set to False it will be removed from the processing list."
            )
            samples_dir.remove(sample_dir)
    # else:
    #     continue
...
print(
    f"{len(samples_dir)} samples folder were found to be complete and will be processed."
)
...
adducts_df["min"] = adducts_df["adduct_mass"] - int(ppm_tol_ms1) * (
    adducts_df["adduct_mass"] / 1_000_000
)
adducts_df["max"] = adducts_df["adduct_mass"] + int(ppm_tol_ms1) * (
    adducts_df["adduct_mass"] / 1_000_000
)
...
if taxo_db_metadata_path.endswith(".csv.gz"):
    ...
    db_metadata = pd.read_csv(
        taxo_db_metadata_path, sep=",", on_bad_lines="skip", low_memory=False
    )
...
for sample_dir in samples_dir:
    ...
    spectra_file_path = os.path.join(
        repository_path, sample_dir, sample_dir + "_features_ms2" + ".mgf"
    )
    ...
    try:
        for file in os.listdir(
            os.path.join(repository_path, sample_dir, "taxo_output")
        ):
            if file.endswith("_taxo_metadata.tsv"):
                taxo_metadata_path = os.path.join(
                    repository_path, sample_dir, "taxo_output", file
                )
            else:
                pass
    ...
    isdb_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/{sample_dir}_isdb.tsv"
    )
    mn_ci_ouput_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/molecular_network/{sample_dir}_mn_metadata.tsv"
    )
    repond_table_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/{sample_dir}_isdb_reweighted.tsv"
    )
    repond_table_flat_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/{sample_dir}_isdb_reweighted_flat.tsv"
    )
    mn_graphml_ouput_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/molecular_network/{sample_dir}_mn.graphml"
    )
    treemap_chemo_counted_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/{sample_dir}_treemap_chemo_counted.html"
    )
    treemap_chemo_intensity_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/{sample_dir}_treemap_chemo_intensity.html"
    )
    isdb_config_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/isdb/config.yaml"
    )
    mn_config_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/molecular_network/config.yaml"
    )
    isdb_folder_path = os.path.normpath(f"{repository_path}/{sample_dir}/isdb/")
    mn_folder_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/molecular_network/"
    )
    ...
    print(
        ...
        + str(len(dt_isdb_results["feature_id"].unique()))
    )
    ...
    print(
        ...
        + str(len(df_MS1["feature_id"].unique()))
    )
    ...
    if taxo_metadata is not None:
        ...
        taxo_reweight = True
        cols_att = [
            "otl_organism_otol_domain",
            "otl_organism_otol_kingdom",
            "otl_organism_otol_phylum",
            "otl_organism_otol_class",
            "otl_organism_otol_order",
            "otl_organism_otol_family",
            "otl_organism_otol_genus",
    ...
    if len(dt_isdb_results) != 0:
        ...
        dt_isdb_results_chem_rew = chemical_reponderator(
            clusterinfo_summary_file=clusterinfo_summary,
        ...
        dt_taxo_chemo_reweighed_topN = top_N_slicer(
            input_df=dt_isdb_results_chem_rew, top_to_output=top_to_output
        )

        if taxo_reweight:
            df_flat, df_for_cyto = annotation_table_formatter_taxo(
                dt_taxo_chemo_reweighed_topN, min_score_taxo_ms1, min_score_chemo_ms1
            )
        else:
            df_flat, df_for_cyto = annotation_table_formatter_no_taxo(
                dt_taxo_chemo_reweighed_topN, min_score_taxo_ms1, min_score_chemo_ms1
            )
        ...
        plotter_count(
            df_flat, sample_dir, organism_label, treemap_chemo_counted_results_path
        )
        plotter_intensity(
            df_flat,
            feature_intensity_table_formatted,
            sample_dir,
            organism_label,
