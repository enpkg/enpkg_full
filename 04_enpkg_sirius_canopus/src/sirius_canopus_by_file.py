...
sirius_version_str = (
    subprocess.check_output([path_to_sirius, "--version"]).decode().split("\n")
)
...
def compute_sirius_canopus(file, output_name):
    subprocess.run(
        sirius_command.format(file=file, output_name=output_name),
        shell=True,
        env=my_env,
    )
    ...
    if metadata["sample_type"][0] == "sample":
        if ionization == "pos":
            sirius_mgf_path = os.path.join(
                path, directory, ionization, directory + "_sirius_pos.mgf"
            )
        elif ionization == "neg":
            sirius_mgf_path = os.path.join(
                path, directory, ionization, directory + "_sirius_neg.mgf"
            )
        else:
            raise ValueError("ionization parameter must be pos or neg")

        output_folder = os.path.join(
            path, directory, ionization, directory + "_" + output_suffix
        )
            ...
            if zip_output:
                for dir in [directory for directory in os.listdir(output_folder)]:
                    if os.path.isdir(os.path.join(output_folder, dir)):
                        shutil.make_archive(
                            os.path.join(output_folder, dir),
                            "zip",
                            os.path.join(output_folder, dir),
                        )
