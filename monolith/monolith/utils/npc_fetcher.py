...
def process_npc_classification(df, inchikey_column='structure_inchikey', smiles_column='structure_smiles', batch_size=1000, db_name='npc_classification.duckdb'):
    # Initialize DuckDB
    ...
    with tqdm(total=total_batches, desc="Processing batches") as outer_pbar:
        ...
        for start in range(0, num_rows_to_process, batch_size):
            ...
            with tqdm(total=len(batch), desc=f"Processing batch {start//batch_size + 1}", leave=False) as inner_pbar:
                for index in range(len(batch)):
                    ...
                    if npc_result:
                        npc_pathway = ', '.join(npc_result['pathway_results']) if npc_result['pathway_results'] else None
                        npc_superclass = ', '.join(npc_result['superclass_results']) if npc_result['superclass_results'] else None
                        npc_class = ', '.join(npc_result['class_results']) if npc_result['class_results'] else None
                        results.append((inchikey, smiles, npc_pathway, npc_superclass, npc_class))
...
if __name__ == "__main__":
    ...
    process_npc_classification(metadata_unique_ik, inchikey_column='structure_inchikey', smiles_column='structure_smiles', batch_size=1000, db_name='npc_classification.duckdb')