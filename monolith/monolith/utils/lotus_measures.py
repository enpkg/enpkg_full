...
def unique_inchikey_pathway(metadata):
    metadata_unique = metadata.drop_duplicates(subset=['structure_inchikey', 'structure_taxonomy_npclassifier_01pathway'])
...
def count_inchikey_with_multiple_pathways(df):
    ...
    pathway_counts = df.groupby('structure_inchikey')['structure_taxonomy_npclassifier_01pathway'].nunique()
...
def inchikey_with_multiple_pathways(df):
    ...
    inchikey_multiple_pathways = inchikey_multiple_pathways.merge(df[['structure_inchikey', 'structure_smiles', 'structure_taxonomy_npclassifier_01pathway']], on='structure_inchikey', how='left')
...
def get_npc_classification_from_metadata(metadata, smiles):
    npc_classification = metadata[metadata['structure_smiles'] == smiles]['structure_taxonomy_npclassifier_01pathway']
    
    return npc_classification

get_npc_classification_from_metadata(metadata_unique, 'C=CC(C)(C)n1cc(CC(NC)C(=O)O)c2ccccc21')
...
def add_npc_classification(df, smiles_column='structure_smiles'):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if npc_result:
            ...
            df.at[index, 'npc_pathway'] = ', '.join(npc_result['pathway_results']) if npc_result['pathway_results'] else None
            df.at[index, 'npc_superclass'] = ', '.join(npc_result['superclass_results']) if npc_result['superclass_results'] else None
            df.at[index, 'npc_class'] = ', '.join(npc_result['class_results']) if npc_result['class_results'] else None
