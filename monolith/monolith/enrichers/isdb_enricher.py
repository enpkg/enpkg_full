...
class ISDBEnricher(Enricher):
    ...
    def __init__(
        self,
        configuration: ISDBEnricherConfig
    ):
        ...
        self._taxo_db_metadata = pd.read_csv("downloads/taxo_db_metadata.csv", low_memory=False)

        downloader.download(
            self.configuration.urls.spectral_db_pos_url,
            "downloads/spectral_db_pos.pkl"
        )
    ...
    def enrich(self, analysis: Analysis) -> Analysis:
        ...
        similarities = calculate_scores(
            ...
            analysis.tandem_mass_spectra,
            similarity_function=ModifiedCosine(tolerance=self.configuration.networking_params.mn_msms_mz_tol),
            is_symmetric=True
        )
        ...
        cosinegreedy = CosineGreedy(tolerance=self.configuration.spectral_match_params.msms_mz_tol)
        ...
        for min_range in trange(
            ...
            spectra_chunk = analysis.annotated_tandem_mass_spectra[min_range:min_range+range_size]

            peak_processed_spectra = [
                peak_processing(spectrum.spectrum)
            ...
            for (x,y) in tzip(
                ...
                if x<y:
                    msms_score, n_matches = cosinegreedy.pair(
                        peak_processed_spectra[x],
                        self._spectral_db_pos[y]
                    )[()]
                    if (
                        msms_score>self.configuration.spectral_match_params.min_score and
                        n_matches>self.configuration.spectral_match_params.min_peaks
                    ):
                        spectra_chunk[x].add_annotation(ChemicalAnnotation(
                            cosine_similarity=msms_score,
                            number_of_matched_peaks=n_matches,
                            molecule_id=y+1,
                            short_inchikey=self._spectral_db_pos[y].get("compound_name")
                        ))
