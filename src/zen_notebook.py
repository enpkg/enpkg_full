...
ACCESS_TOKEN_SANDBOX = pystow.get_config("zenodo", "sandbox_api_token", raise_on_missing=True)
...
for plate in tqdm(['VGF138', 'VGF139', 'VGF140', 'VGF141', 'VGF142',
    ...
    r = requests.post('http://sandbox.zenodo.org/api/deposit/depositions',
                    params=params,
                    json={},
                    headers=headers)
...
for record in results['hits']['hits']:
    if ('Individual .ttl files for plate' in record['metadata']['title']) & ('v1.0.1' in record['metadata']['title']):
